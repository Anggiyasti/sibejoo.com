...
class Donasiback extends CI_Controller{
	...
	function set_siswa_donasi(){
		if ($this->input->post()) {
			...
			// // SELECT SISWA YANG SUDAH KONFIRM => fungsi ini sudah tidak digunakan
			// $id_siswa_jenis_donasi = $this->donasiback_model->get_id_siswa($id_donasi)[0];
			// SELECT TOKEN YANG SESUAI DAN MASIH KOSONG
			$token_id = $this->donasiback_model->select_token($jenis_donasi);
			if ($token_id==array()) {
				echo json_encode(array('status'=>0,'message'=>"Maaf sisa token kosong"));
			}else{
				$update = ['id_donasi'=>$id_donasi,'id_token'=>$token_id[0]->token_id,'id_siswa'=>$id_donasi];
			// UPDATE TOKEN
	...
	function ajax_datatatable_donasi(){
		...
		foreach ( $data['donasi_items'] as $item ) {
			...
			if ($item->status_donasi==1){
				...
				$button = '<button  onclick="set_token('.$item->donasi_id.','.$item->donasi.')" type="button" class="btn btn-primary mb5" ><i class="ico-barcode3"></i> Set Token</button>';
			}else{
