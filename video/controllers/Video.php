...
class Video extends MX_Controller {
    ...
    public function videobysub($sub_bab_id) {
        ...
        if ($data['judulbab'] == array()) {
            ...
            $data = array(
                'judul_halaman' => 'Sibejoo - Sub : ' . $judul_halaman,
...
// tampilkan ini matapelajaran apa dan untuk tingkat apa.
        ...
        $data = array(
            'judul_halaman' => 'Sibejoo - Video Pelajaran ' . $data['meta']['aliasMataPelajaran'],
    ...
    if ($data['videosingle'] == array()) {
        ...
        $data['files'] = array(
            APPPATH . 'modules/homepage/views/r-header-login.php',
            APPPATH.'modules/templating/views/t-f-pagetitle.php',
            APPPATH . 'modules/video/views/r-single-video.php',
                APPPATH . 'modules/testimoni/views/r-footer.php'
            );
        $this->parser->parse('templating/r-index', $data);
    }
...
function ajax_get_last_video() {
    ...
    $row[] = "<li><a onclick='kunjungivideo(".$list_video['id'].")'> 
        <i class='fa fa-play mr-10 text-black-light'></i> ".$list_video['judulVideo']."</a></li>";
