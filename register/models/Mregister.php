...
//merupakan function untuk menyimpan data guru ke tabel siswa di databse Neon  
  ...
  public function insert_orangtua($data_ortu)
  {
    $this->db->insert('tb_orang_tua', $data_ortu);
  }
...
//untuk mngambil nilai id penngguna untuk digunakan FK di tb_siswa
  ...
  public function get_idSiswa($data)
  {
    $this->db->select('siswa.id as siswaID');
    $this->db->from("tb_pengguna user");
    $this->db->join("tb_siswa siswa","siswa.penggunaID=user.id");
    $this->db->where('user.namaPengguna', $data);
    $query = $this->db->get();
    return $query->result_array();
  }
