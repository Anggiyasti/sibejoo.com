...
class Learningline extends MX_Controller {
	...
	public function edit_step($data){
		...
		if ($metadata['jenisStep']==1) {
			...
			// $soal = $this->learning_model->get_soal_tambah($data);
			// var_dump($soal);
		}
...
# detail pada saat di klik ##
	...
	function get_soal_on_latihan($stepID){

		$list = $this->learning_model->get_soal_tambah($stepID);
		$data = array();
	...
	function ajax_get_soal_edit($bab,$step) {
		$param = ['bab_id'=>$bab, 'step_id'=>$step];

		$list = $soal=$this->mlatihan->get_soal_bybab( $param );
		$data = array();
