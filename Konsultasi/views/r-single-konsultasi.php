...
<section>
				...
				<div class="col-xs-12 col-sm-12 col-md-12 pb-sm-20 mb10">
						...
						<article>
							...
							<div class="quotes clear-fix" >
								<div class="quote-avatar-author clear-fix">
									<img src="{photo}" data-at2x="{photo}" title="{namaPengguna}" width="60px">
					...
					<?php if ($data_postingan!=array()): ?>
						<?php foreach ($data_postingan as $item_postingan): ?>
							<?php $link = "show_post(".$item_postingan['jawabID'].")" ?>
							<?php $number++; ?>
								...
								<article>
									<div class="row bg-color-2">
										<div class="container"><?=$item_postingan['date_created'] ?> |
											<a title="view single post" onclick="show_post(<?=$item_postingan['jawabID']?>)">#<?=$number ?></a></div>
										</div><br>
										...
										<div class="quotes clear-fix" >
											...
											<div>
												...
												"<a title='view single post' onclick='".$link."'><i class='fa fa-arrow-circle-o-right'> > </i></a>"?>
						...
						<div class="container">	
							...
							<div class="col-sm-12" id="jawaban">
								...
								<span class="loading_posting"></span>
								<br>
	...
	<script type="text/javascript">
		...
		function append_new(datas){
			...
			function simpan_jawaban(){
				$('#post_btn').hide();
					jQuery(".loading_posting").html('<div style="padding:10px"><img src="<?php echo base_url('assets/image/loading/spinner11.gif'); ?>" alt="Please Wait"/> <span>Posting...</span></div>');
			...
			function get_data(data, datas){
				...
				if (status==1) {
					...
					$.ajax({
						...
						success: function()
						{
							swal('Yeah',"Point sudah diberikan",'success');
							$('#myModal').modal('toggle');
						},
						error: function (jqXHR, textStatus, errorThrown)
						{
	...
	function show_post(post_id) {
		url_ajax = base_url+"konsultasi/tamp_single_jawab";
