...
class Mlinetopik extends CI_Model
{
  ...
  public function get_mapel($tingkatID)
  {
    ...
    if ($this->session->userdata('member')==0) {
        $this->db->where('bab.statusAksesLearningLine',0);
 }
...
// mengambil akses learning line berdasarkan bab.
    public function get_akses_learning_line($babID){
        $this->db->where('id', $babID);
        $this->db->select('statusAksesLearningLine');
        $this->db->from('tb_bab');
        ...
        if ($result->result_array()==array()) {
            ...
            return $result->result_array()[0]['statusAksesLearningLine'];
        }
    }
